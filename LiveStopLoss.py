...
class LiveStopLoss:
    ...
    def set_stop_loss(self, position, stop_loss_price=None):
        ...
        if position['type'] == mt5.ORDER_TYPE_SELL:
            ...
            if position['tp'] == 0:
                tp_price = position['price_open'] - self.config['ratio_sl_tp'] * self.config['stop_loss'] * self.point
    ...
    def trail_stop_loss(self, position):
        if position['type'] == mt5.ORDER_TYPE_SELL:
            ...
            print(profit_point ,  self.sl_point)
            #print('Update %d' % profit_point, end='\r')

            if profit_point >= 1 * self.sl_point:
                new_sl_price = position['price_current'] + self.sl_point
                print(new_sl_price, self.last_sl_price)
            ...
            while len(self.get_opened_positions()) != 0:
                ...
                for position in positions:
                    if position['sl'] == 0 or position['tp'] == 0:
                        ...
                        self.sl_point = self.config['stop_loss'] * self.point
                        self.tp_point = self.config['ratio_sl_tp'] * self.sl_point
